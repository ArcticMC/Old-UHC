		...
		if (cmd.getName().equals("ms")) {
			if (args.length == 0) {
				if (sender instanceof Player) {
					...
					player.sendMessage(Main.prefix() + "Your ping: §6" + PlayersUtil.getPing(player));
				} else {
			...
			sender.sendMessage(Main.prefix(ChatColor.GOLD) + target.getName() + "'s §7ping: §6" + PlayersUtil.getPing(target));
		}
		return true;
	}
}