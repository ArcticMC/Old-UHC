		...
		if (cmd.getName().equalsIgnoreCase("scenario")) {
			...
			if (sender.hasPermission("uhc.scenarioadmin")) {
				...
				if (args[0].equalsIgnoreCase("enable")) {
					...
					PlayersUtil.broadcast(Main.prefix(ChatColor.GOLD) + s.getName() + " §7has been enabled.");
					s.setEnabled(true);
					...
					PlayersUtil.broadcast(Main.prefix(ChatColor.GOLD) + s.getName() + " §7has been disabled.");
					s.setEnabled(false);
