...
public class ConfigCommand implements CommandExecutor {
		...
		if (cmd.getName().equalsIgnoreCase("config")) {
			if (args.length > 1 && args[0].equalsIgnoreCase("set")) {
				if (sender.hasPermission("uhc.config.set")) {
					...
					try {
						switch (type) {
							...
							case MEETUP:
								...
								Settings.getInstance().getData().set("game." + args[1].toLowerCase(), b);
								Settings.getInstance().saveData();
								if (GameState.isState(GameState.INGAME)) {
									Scoreboards.getManager().setScore("§b§lMeetup", Integer.parseInt("-" + b));
								}
								sender.sendMessage(Main.prefix() + "Set " + args[1].toLowerCase() + " to " + args[2] + ".");
								break;
							case MOTD:
							...
							case PVP:
								...
								Settings.getInstance().saveData();
								if (GameState.isState(GameState.INGAME)) {
									Scoreboards.getManager().setScore("§b§lPvP", Integer.parseInt("-" + p));
								}
								sender.sendMessage(Main.prefix() + "Set " + args[1].toLowerCase() + " to " + args[2] + ".");
								break;
							case SHEARRATE:
					...
					sender.sendMessage(Main.absorbtion ? "§7- §fAbsorption is on" : "§7- §fAbsorption is off");
					...
					sender.sendMessage("§7- §fApple rates are " + Main.applerate);
	...
	public enum ConfigType {
		FFA, PVP, MEETUP, HOST, FLINTRATE, APPLERATE, SHEARRATE, TEAMSIZE, CURRENTSTATE, ABSORB, GHEAD, PEARLDMG, GODAPPLE, POST, MOTD, MAXPLAYERS, WORLD;
	}
}