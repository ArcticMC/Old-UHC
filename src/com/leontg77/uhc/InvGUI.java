...
public class InvGUI {
	...
	public void openSelector(Player player) {
		Inventory inv = Bukkit.createInventory(null, PlayersUtil.playerInvSize(), "Player Selector");
	...
	public void openInv(Player player, final Player target) {
		...
		ArrayUtil.invsee.put(inv, new BukkitRunnable() {
			public void run() {
				...
				for (PotionEffect l : target.getActivePotionEffects()) {
					lore.add(ChatColor.GRAY + l.getType().getName().substring(0, 1).toUpperCase() + l.getType().getName().substring(1).toLowerCase().replaceAll("_", "") + " §atier: §7" + (l.getAmplifier() + 1) + " §aLength: §7" + TimerRunnable.ticksToString(l.getDuration() / 20));
				}
		...
		ArrayUtil.invsee.get(inv).runTaskTimer(Main.plugin, 1, 1);
