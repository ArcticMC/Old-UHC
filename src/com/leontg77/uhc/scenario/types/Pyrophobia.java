...
public class Pyrophobia extends Scenario implements Listener {
	...
	public void onBlockIgnite(BlockIgniteEvent event) {
		...
		if (cause == IgniteCause.LIGHTNING) {
	...
	public void LeavesDecayEvent(LeavesDecayEvent event) {
		...
		Random r = new Random();
		if (r.nextInt(100) < 2) {
			event.getBlock().getWorld().dropItemNaturally(event.getBlock().getLocation(), new ItemStack(Material.SUGAR_CANE, 1 + r.nextInt(1)));
		}
	}
	...
	protected void completedForReal() {
		Bukkit.getServer().getScheduler().cancelTask(this.generateTaskID);
		this.generateTaskID = -1;
		Bukkit.getServer().broadcastMessage("World Converted");
	}
		...
		for (int y = 0; y < 128; y++) {
			for (int x = 0; x < 16; x++) {
				for (int z = 0; z < 17; z++) {
					...
					if ((b.getType() == Material.REDSTONE_ORE) && (r.nextInt(50) < 4)) {
						chunkAt.getBlock(x, y, z).setType(Material.OBSIDIAN);
