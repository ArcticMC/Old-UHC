...
public class Cryophobia extends Scenario implements Listener {
	...
	public void startCryo() {
		...
		this.theworld = Main.host().getWorld();
	...
	protected void processChunkIfRequired() {
		...
		for (int a = 0; a < attempts; a++) {
			...
			if ((cp != null) && (cp.x >= 0) && (cp.x < 500) && (cp.z >= 0)
					&& (cp.z < 500)
					&& (this.existingHeights[cp.x][cp.z] < this.levelHeight)) {
				World w = theworld;
				if (w != null) {
					...
					if (c != null) {
						...
						if ((cp.stopHeight < this.levelHeight)
								&& (this.levelHeight - upToHeight < 8)) {
							cp.stopHeight = this.levelHeight;
						}
						for (int y = upToHeight; y <= cp.stopHeight; y++) {
							for (int i = 0; i < 16; i++) {
								for (int j = 0; j < 16; j++) {
									if (!c.getBlock(i, y, j).getType()
											.isSolid()) {
										c.getBlock(i, y, j).setType(
	...
	protected void endRaiseTimer() {
		if (this.heightUpdateTimer != -1) {
			Bukkit.getServer().getScheduler()
					.cancelTask(this.heightUpdateTimer);
			this.heightUpdateTimer = -1;
		}
		if (this.processTimer != -1) {
			Bukkit.getServer().getScheduler()
					.cancelTask(this.processTimer);
			this.processTimer = -1;
		}
	}
	...
	public void onEntityShootBowEvent(EntityShootBowEvent event) {
		...
		if (event.getProjectile().getType() == EntityType.ARROW) {
			...
			event.getEntity().launchProjectile(Snowball.class,
					event.getProjectile().getVelocity());
		}
	}
